...
class QueuePieces:
    ...
    def to_view(self):
        obj = {i:
               {y:
                {x: self.pieces[i].shape[y][x]
                    for x in range(len(self.pieces[i].shape[y]))}
    ...
    def fill(self, color, shape_number):
        self.pieces = [Piece(color=color, shape_number=shape_number)
                       for i in range(self.size)]
